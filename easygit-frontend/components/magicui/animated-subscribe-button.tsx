...
>(
  (
    { subscribeStatus = false, onClick, className, children, showBorderBeam = true, ...props },
    ref,
  ) => {
      ...
      <AnimatePresence mode="wait">
        {isSubscribed ? (
          <motion.button
            ref={ref}
            className={cn(
              "relative flex h-10 w-fit items-center justify-center overflow-hidden rounded-lg bg-white  px-6 text-black",
              className,
            )}
          ...
          >
            <motion.span
              key="action"
              className="relative flex h-full w-full items-center text-black justify-center font-semibold"
              initial={{ y: -50 }}
              animate={{ y: 0 }}
            >
              {changeChild} {/* Use children for subscribed state */}
            </motion.span>
            {showBorderBeam && <BorderBeam borderWidth={2}/>}
          </motion.button>
        ) : (
          <motion.button
            ref={ref}
